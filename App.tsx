...
const IntroAnimation: React.FC<{ onFinish: () => void }> = ({ onFinish }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      // Change phrase every 1.5 seconds roughly
      if (currentStep % 30 === 0) {
         setPhrase(phrases[Math.floor(Math.random() * phrases.length)]);
      }
      ...
      if (currentStep >= steps) {
        clearInterval(timer);
        setTimeout(onFinish, 500); // Small delay at 100% before unmounting
      }
    }, intervalTime);
    ...
    <div className="fixed inset-0 z-[100] bg-[#020617] flex flex-col items-center justify-center font-sans overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="w-64 h-1 bg-white/10 rounded-full overflow-hidden relative">
           <div 
             className="h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 transition-all duration-100 ease-linear shadow-[0_0_10px_rgba(99,102,241,0.5)]"
             style={{ width: `${progress}%` }}
           ></div>
        </div>
        <p className="mt-2 text-[10px] text-slate-500 font-mono">{Math.round(progress)}%</p>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="relative z-50">
      <button 
        ...
        className="flex items-center gap-2 bg-[#0B0F19]/50 backdrop-blur-md border border-white/10 px-3 py-2 rounded-full hover:bg-white/10 transition-colors"
      >
        ...
        <div className="absolute top-full right-0 mt-2 w-32 bg-[#0B0F19] border border-white/10 rounded-xl shadow-2xl overflow-hidden py-1">
          {languages.map((l) => (
            <button
              ...
              className={`w-full text-left px-4 py-2 text-xs font-medium hover:bg-white/5 flex items-center gap-2 ${current === l.code ? 'text-white bg-white/5' : 'text-slate-400'}`}
            >
...
const SellfyApp: React.FC = () => {
  ...
  // Estos hooks dependen del ID del usuario. Si no hay sesión, no hacen nada.
  const { 
    settings, 
    subscription, 
    updateSettings, 
  ...
  // Handle Intro Logic
  useEffect(() => {
    if (session && !authLoading && !introSeen) {
      setShowIntro(true);
    }
  }, [session, authLoading, introSeen]);

  // 1. Loading Screen (Initial Supabase check)
  if (authLoading) {
    return (
  ...
  // 2. Login Screen (Si no hay sesión)
  if (!session) {
    return <LoginView />;
  }
  ...
  if (showIntro) {
    return <IntroAnimation onFinish={() => { setShowIntro(false); setIntroSeen(true); }} />;
  }
      ...
      <div className="flex-1 flex flex-col relative z-10 min-w-0">
        ...
        <div className="md:hidden p-4 flex items-center justify-between border-b border-white/5 bg-[#020617]/50 backdrop-blur-md sticky top-0 z-50">
            <span className="font-bold text-lg tracking-tight text-white flex items-center gap-2">
               <Camera size={16} className="text-indigo-400" /> sellfy
            </span>
             <LanguageSwitcher current={lang} setLang={setLang} />
        </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth hide-scrollbar">
          ...
          {currentView === 'history' && (
            <HistoryView 
              ...
              isFreePlan={subscription.plan === 'free'} // Pass plan info
            />
          )}
          ...
          {currentView === 'wizard' && (
            <WizardView 
              ...
              onCampaignCreated={saveCampaign} // Guarda en Supabase
              onViewImage={setFullScreenImage}
              apiKeyMissing={apiKeyMissing}
              subscription={subscription}
              onDecrementCredit={decrementCredits} // Resta crédito en Supabase
            />
          )}
        </main>
      </div>
